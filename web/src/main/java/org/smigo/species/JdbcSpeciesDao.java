...
class JdbcSpeciesDao implements SpeciesDao {
    ...
    public List<Species> searchSpecies(String search, Locale locale) {
        ...
        final String whereClause = "SPECIES.ID IN (SELECT SPECIES_ID FROM SPECIES_TRANSLATION WHERE VERNACULAR_NAME LIKE ?) OR SCIENTIFIC_NAME LIKE ? OR FAMILIES.NAME LIKE ? OR SPECIES.ID LIKE ?";
