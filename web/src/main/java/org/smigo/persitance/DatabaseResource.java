...
public class DatabaseResource implements Serializable {
  ...
  @Autowired
  public DatabaseResource(DataSource datasource, MessageSource messageSource) throws Exception {
    log.debug("Creating a new databaseresource with datasource" + datasource.toString());
    if (datasource instanceof BoneCPDataSource) {
      BoneCPDataSource bcpds = (BoneCPDataSource) datasource;
      log.debug("JdbcUrl:" + bcpds.getConfig().getJdbcUrl());
    }
  ...
  public int addSpecies(Species species) {
    log.info("Adding " + species + " for user:" + species.getCreator().getUsername());
    Connection con = null;
    ...
    try {
      ...
      insertSpecies.setString(1, species.getScientificName());
      insertSpecies.setBoolean(2, false);
      insertSpecies.setBoolean(3, species.isAnnual());
      insertSpecies.setInt(4, species.getFamily().getId());
      insertSpecies.setBoolean(5, false);
      insertSpecies.setInt(6, species.getCreator().getId());
      insertSpecies.setString(7, "defaulticon.png");
      insertSpecies.execute();
      rs = insertSpecies.getGeneratedKeys();
      rs.next();
      species.setId(rs.getInt(1));
    } catch (Exception e) {
      log.error("Could not insert " + species, e);
    ...
    updateUserSettingsForSpecies(species, species.getCreator());
  ...
  public void updateUserSettingsForSpecies(Species species, User user) {
    log.info("Updating user setting species:" + species + " user:" + user);
    Connection con = null;
    ...
    try {
      ...
      ps.setString(4, species.getTranslation());
      ps.setString(5, species.getScientificName());
      ps.setString(6, species.getTranslation());
      ps.executeUpdate();
    } catch (Exception e) {
      log.error("Could not update usersettings for species " + species, e);
      throw new RuntimeException(e);
    } finally {
      close(con, ps);
    }
    // updating visibility
    setSpeciesVisibility(species.getId(), user, true);
  }
  ...
  public Map<Integer, Species> getSpecies(User user) {
    // Getting species
    HashMap<Integer, Species> ret = new HashMap<Integer, Species>(200);
    Connection con = null;
    ...
    try {
      ...
      while (speciesRS.next()) {
        Species species = new Species(speciesRS.getInt("species_id"),
                                       speciesRS.getString("scientificname"), speciesRS.getString("translation"),
                                       speciesRS.getBoolean("item"), speciesRS.getBoolean("annual"),
                                       new Family(speciesRS.getString("familyname"), speciesRS.getInt("familyid")));
        species.setDisplay(speciesRS.getBoolean("display"));
        species.setIconFileName(speciesRS.getString("iconname"));
        if (speciesRS.getInt("creator") == user.getId())
          species.setCreator(user);
        // if user has no translation, use messageSource
        if (species.getTranslation() == null)
          species.setTranslation(messageSource.getMessage(species.getTranslationKey(),
                                                           null, species.getScientificName(), user.getLocale()));
        ret.put(species.getId(), species);
      }
      ...
      while (ruleRS.next()) {
        Species host = ret.get(ruleRS.getInt("host"));
        Species causer = ret.get(ruleRS.getInt("causer"));
        ...
        Rule rule = ruleFactory.createRule(ruleRS.getInt("rule_id"),
                                            ...
                                            ruleRS.getBoolean("display"), ruleRS.getInt("creator"), family, messageSource, user);
  ...
  public List<PlantDb> getPlants(User user) {
    List<PlantDb> ret = new ArrayList<PlantDb>(1000);
    if (user.getId() == 0)
      return ret;
    Connection con = null;
    PreparedStatement ps = null;
