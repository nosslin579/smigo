 ...
 * {@link org.springframework.util.PropertiesPersister} strategy: The default
 * strategy is capable of loading properties files with a specific character
 * encoding, if desired.
 * <p/>
 * <p/>
...
public class ReloadableResourceBundleMessageSource2 extends AbstractMessageSource implements
  ResourceLoaderAware {
  ...
  /**
   * Cache tod hold merged loaded properties per basename
   */
  private final Map cachedMergedProperties = new HashMap();

  public ReloadableResourceBundleMessageSource2() {
   ...
   * "WEB-INF/messages.properties", etc.
   * <p/>
   * As of Spring 1.2.2, XML properties files are also supported: e.g.
   ...
   * "WEB-INF/messages.properties", "WEB-INF/messages.properties", etc.
   * <p/>
   * As of Spring 1.2.2, XML properties files are also supported: e.g.
   * "WEB-INF/messages" will find and load "WEB-INF/messages.xml",
   * "WEB-INF/messages_en.xml", etc as well. Note that this will only work on
  ...
  protected PropertiesHolder getProperties(String filename) {
    synchronized (this.cachedProperties) {
      ...
      if (propHolder != null
            && (propHolder.getRefreshTimestamp() < 0 || propHolder.getRefreshTimestamp() > System
    ...
    if (resource.exists()) {
      ...
      try {
        ...
        if (logger.isWarnEnabled()) {
          logger.warn("Could not parse properties file [" + resource.getFilename() + "]",
                       ex);
        }
  ...
  public void clearCacheIncludingAncestors() {
    clearCache();
    if (getParentMessageSource() instanceof ReloadableResourceBundleMessageSource2) {
      ((ReloadableResourceBundleMessageSource2) getParentMessageSource())
