    ...
    var w = d.module("ngRoute", ["ng"]).provider("$route", function () {
        ...
        this.$get = ["$rootScope", "$location", "$routeParams", "$q", "$injector", "$templateRequest",
            "$sce", function (a, c, b, f, h, k, x) {
                ...
                function m() {
                    var v = s.current, e = p;
                    if (w)v.params = e.params, d.copy(v.params, b), a.$broadcast("$routeUpdate", v); else if (e || v)u = !1, (s.current = e) && e.redirectTo && (d.isString(e.redirectTo) ? c.path(t(e.redirectTo, e.params)).search(e.params).replace() : c.url(e.redirectTo(e.pathParams,
